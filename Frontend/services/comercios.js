...
//Me devuelve las credenciales de un comercio
...
export const getComercioAuth = async (uid_comercio) => {
  try {
    ...
    if (!result.ok) {
      ...
      throw new Error(
        errorData.message || `Error ${result.status}: ${result.statusText}`
      );
    }
    return await result.json();
  } catch (error) {
    console.error("Error al obtener Autorizacion de Comercio:", error);
    throw error;
