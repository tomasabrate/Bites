      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          text: "Confirmar", onPress: () => {
            setPedidos((prevPedidos) =>
              prevPedidos.map((pedido) =>
                pedido.id === id ? { ...pedido, estado: 'pedido retirado' } : pedido
              )
            );
            Alert.alert('Éxito', `El pedido ${id} ha sido pasado a estado 'pedido retirado'.`);
          }
        }
      ]
    );
        ...
        renderItem={({ item }) => (
          <View style={styles.pedido}>
            ...
            <View style={styles.estadoContainer}>
              ...
              <Text style={[styles.estadoText, { backgroundColor: getEstadoColor(item.estado) }]}>
                {item.estado}
              </Text>
            </View>
            {item.estado === 'pedido retirado' ? (
              <View style={styles.inputContainer}>
                ...
                />
                <TouchableOpacity style={styles.boton} onPress={() => marcarComoEntregado(item.id)}>
                  <Text style={styles.botonTexto}>Marcar como Entregado</Text>
                </TouchableOpacity>
              </View>
            ) : item.estado === 'pendiente de entrega' ? (
              <TouchableOpacity style={styles.boton} onPress={() => confirmarCambioEstado(item.id)}>
