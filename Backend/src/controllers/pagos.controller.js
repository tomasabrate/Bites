...
export const getPagos = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM Pagos ORDER BY fecha_creacion DESC');
    res.json(rows);
  } catch (error) {
...
export const getPagoById = async (req, res) => {
  try {
    ...
    if (rows.length <= 0) return res.status(404).json({ message: 'Pago not found' });
    console.log(rows[0]);
...
export const getPagoByPaymentId = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT * FROM Pagos WHERE payment_id = ?', [payment_id]);
    if (rows.length <= 0) return res.status(404).json({ message: 'Pago not found' });
    return res.status(200).json(rows[0]);
  } catch (error) {
...
export const updatePagoVenta = async (id_venta, payment_id) => {
  try {
    const [result] = await pool.query('UPDATE Pagos SET id_venta = ? WHERE payment_id = ?', [id_venta, payment_id]);
...
export const updateEstadoPago = async (estado, payment_id) => {
  try {
    const [result] = await pool.query('UPDATE Pagos SET fecha_actualizacion = NOW(), estado = ? WHERE id_pago = ?', [estado, payment_id]);
    if (result.affectedRows === 0) {
      return "ERROR -- Pago no encontrado";
    } else {
...
export const deletePago = async (req, res) => {
  try {
    const { id_pago } = req.params;
    const [result] = await pool.query('DELETE FROM Pagos WHERE id_pago = ?', [id_pago]);
    if (result.affectedRows === 0) return res.status(404).json({ message: 'Pago not found' });
